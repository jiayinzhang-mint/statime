...
use observer::{json::JsonObserver, ObservableInstanceState, Observer};
use port::{eth::ethernet_port_task, ip::port_task, BmcaPort};
...
pub async fn start_sync(config: Config, observer: Arc<JsonObserver>) {
    ...
    .await
    .inspect_err(|err| log::error!("Error: {err}, Abnormal termination of the state observer"));
